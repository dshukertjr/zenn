...
最後に、自分のチャットと他のユーザーのチャットとで画面の右と左どちらに寄せるかを変えましょう。これが地味にトリッキーで、ただ単に右よせ、左寄せをするだけでなく、各要素の左右の並び順を変えないと自然なレイアウトが作れなかったりします。そんな、場合わけで左右の並び順を変えるようなレイアウトも変数に一度各要素を入れて、`List`の`reversed`を使って逆順にすることで実現できます。
